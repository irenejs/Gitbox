...
public class JGitSyncProvider implements ISynchronizationProvider {
	...
	public boolean localExists() {
		...
		try (Repository repository = getRepo(localDirPath.toString())) {
	...
	public void localCreateNew() {
		...
		try (Git git = Git.init().setDirectory(localDirPath.toFile()).call()) {
	...
	public boolean remoteExists() {
		...
		try {
			List<RemoteConfig> remoteList = repository.remoteList().call();
			boolean result = !remoteList.isEmpty();
